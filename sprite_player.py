	...
	while True:
		...
		if (capture.get(cv2.CAP_PROP_POS_FRAMES)) == 1:
			...
			if deinterlace_flag:
				...
				ret2, th2 = thresh_func(current_frame, mode2, threshold, block2, substract2)
				previous_sections2, previous_count2 = split_frame(th2, n_rows,n_columns)
				ret3, th3 = thresh_func(current_frame, mode2, threshold, block2, substract2)
				previous_sections3, previous_count3 = split_frame(th3, n_rows,n_columns)
			continue
		...
		if not calibration_flag:
			ret1, th1 = thresh_func(th1, mode, threshold, block, substract, close, open)
		...
		for rows in range(n_rows):
			...
			if deinterlace_flag:
				print(colored("    ||", "red"), end = "")
				print(" |".join(str(e).rjust(5) for e in delta2[rows*n_columns:(rows+1)*n_columns]), end = "")
				print(colored("    ||", "red"), end = "")
				print(" |".join(str(e).rjust(5) for e in delta3[rows*n_columns:(rows+1)*n_columns]), end = "")
				print()
			else:
				print()
		# if deinterlace_flag:
