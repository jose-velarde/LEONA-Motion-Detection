...
def print_clips_in_folder(rootdir):
...
def deinterlace_clips(folders, deinterlace_flag, narrow_flag):
    deinterlace_mode = "discard"
    # deinterlace_mode = "linear"
    if narrow_flag == True:
        narrow = 2
    ...
    for folder in folders:
        clips_list = print_clips_in_folder(folder)
        ...
        for clip in clips_list:
            ...
            if deinterlace_flag:
                p = "{}Deinterlaced/{} - deinterlaced.avi".format(
                    clip_path[:(100+narrow)], clip_path[(110+narrow):-15]
                )
                writer = cv2.VideoWriter(
                    p, 0, 60, (int(capture.get(3)), int(capture.get(4)))
                )
...
deinterlace_clips(Stations_Narrow, deinterlace_flag, narrow_flag)