...
%     file_nc = strcat(filepath_nc, image_name, '.nc');
    file_nc = strcat(filepath_nc, files(n));
...
%         save(strcat('./data_mat/', files(n), '.mat'), 'plottedTemp');
    end

    %% Plot Cloud Top Temperatures
    if n == 1
        temp_plot = imagesc(temp);
